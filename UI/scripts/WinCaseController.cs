...
public class WinCaseController : MonoBehaviour
{
    ...
    void ShowWinPanel()
    {
        ...
        var canvasGroup = winPanel.GetComponent<CanvasGroup>() ?? winPanel.AddComponent<CanvasGroup>();
        canvasGroup.alpha = 1; canvasGroup.interactable = true; canvasGroup.blocksRaycasts = true;

        var cv = winPanel.GetComponent<Canvas>() ?? winPanel.AddComponent<Canvas>();
    ...
    public void TryAgain()     { Time.timeScale = 1f; SceneManager.LoadScene(gameScene); }
    public void NextLevel()    { Time.timeScale = 1f; SceneManager.LoadScene(gameScene); } // replace with real next-level
